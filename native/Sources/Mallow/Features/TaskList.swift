...
/*
================================================================================================
 INTEGRATION NOTES  (the lead applies these; this file edits nothing on its own)
================================================================================================

This feature plugs into the exact same three seams the `-`→`•` bullet substitution uses: a VM-computed
set, the glyph delegate, and (new) a click recognizer. All offsets below are UTF-16 (NSString domain),
matching `hiddenChars` and `bulletMarks`.

------------------------------------------------------------------------------------------------
 1) EditorViewModel — add a published map + populate it in recomputeHidden()
------------------------------------------------------------------------------------------------
Add a stored property next to `bulletMarks`:

        private(set) var taskBoxes = [Int: Bool]()   // inner-char UTF-16 index → isChecked (glyph delegate)

Then, INSIDE `recomputeHidden()`, alongside the existing bullet loop, do three things using the scanner.
Place this AFTER `let grammar = MarkerGrammar(ns: ns)` and the bullets loop, and BEFORE the
`hiddenChars = collector.hidden` assignment so the bracket-hiding is included in the same invalidation:

        // GFM task-list checkboxes. Render `[ ]`/`[x]` as ☐/☑ off the caret line: hide the two brackets,
        // substitute the inner char (done in the glyph delegate). On the caret line, show the raw box.
        var taskMap = [Int: Bool]()
        let scanner = TaskBoxScanner(s)                       // (s == textView.string; ns == s as NSString)
        for block in blocks where block.kindTag == "List" {
            let bLo = byteToUTF16(s, block.range.start)
            let bHi = min(byteToUTF16(s, block.range.end), total)
            for (inner, checked) in scanner.boxes(in: bLo, bHi) {
                // inner is the ` `/`x` between the brackets; `[`=inner-1, `]`=inner+1.
                let onCaretLine = inner >= caretLine.location
                                && inner <  caretLine.location + caretLine.length
                if onCaretLine { continue }                   // reveal raw `[ ]` on the caret's line
                taskMap[inner] = checked
                collector.hidden.insert(inner - 1)            // hide `[`
                collector.hidden.insert(inner + 1)            // hide `]`
            }
        }
        taskBoxes = taskMap
   ...
   Notes on this block:
   • `collector.hidden` is a `var Set<Int>` on the (mutable) `collector`; inserting directly is fine and
     mirrors how the four hide-passes accumulate into it. (Do NOT also hide `inner` — that char is the one
     the glyph delegate SUBSTITUTES; hiding it would zero-width the checkbox.)
   • The bracket-hiding intentionally respects the caret line via the `onCaretLine` guard ABOVE, so it is
     consistent with `HiddenSyntaxCollector.hideChar`'s caret-line reveal. (We don't route through
     `hideChar` because it's private to the collector; the explicit guard here is equivalent for these
     two indices, and newlines are never adjacent to a box inner char so the newline check is moot.)
   • `total` and `caretLine` already exist as locals in `recomputeHidden()`; reuse them.
   • The marker pass already keeps `[`,`]`,inner VISIBLE (MarkerGrammar.skipTaskBox), so without this we'd
     show literal brackets; here we override two of those three to hidden and the third to a glyph.

   `EditorViewModel.refresh()` and `selectionChanged()` both call `recomputeHidden()`, so the boxes (and
   their caret-line reveal) update on every edit and every caret move automatically — no other VM change.

------------------------------------------------------------------------------------------------
 2) Coordinator glyph delegate — substitute ☐/☑ for the inner char
------------------------------------------------------------------------------------------------
In `MarkdownEditor.Coordinator.layoutManager(_:shouldGenerateGlyphs:...)`, extend the loop to also read
`vm.taskBoxes`. Minimal diff to the existing method:

   • At the top, after `let bullets = vm.bulletMarks`:

        let taskBoxes = vm.taskBoxes
        if hidden.isEmpty && bullets.isEmpty && taskBoxes.isEmpty { return 0 }   // (extend the early-out)
        let boxGlyphs = taskBoxes.isEmpty ? nil : TaskBoxGlyphs(font: font)

   • Inside the per-glyph loop, add a branch BEFORE the `bullets` branch (a checkbox inner char is never
     also a bullet, but order it first for clarity). Replace the existing if/else-if chain body with:

        let ch = characterIndexes[i]
        if hidden.contains(ch) {
            newGlyphs[i] = glyphs[i]; newProps[i] = .null; changed = true
        } else if let checked = taskBoxes[ch], let bg = boxGlyphs, bg.hasGlyphs {
            newGlyphs[i] = bg.glyph(checked: checked)        // ☐ / ☑
            newProps[i]  = props[i]                           // keep normal layout props (visible glyph)
            changed = true
   ...
   Why this is offset-safe: the brackets `[` and `]` are zero-width (.null) via hiddenChars, and the inner
   char keeps its 1 glyph (just a different glyph id). So 3 source chars → 3 glyphs (2 null + 1 box) =
   the same 1:1 char↔glyph the bullet path guarantees; selection, caret, find-ranges, and the
   block-decoration glyph ranges all stay byte-exact.

   Fallback: if the body font lacks ☐/☑ (`bg.hasGlyphs == false`), the box branch is skipped and the
   inner char renders literally — combined with the hidden brackets you'd briefly see just ` `/`x`. If
   that fallback matters, the lead can instead NOT hide the brackets when `!hasGlyphs` (i.e. compute
   `taskBoxes` only when the glyphs exist). In practice the system font has both glyphs.

------------------------------------------------------------------------------------------------
 3) MarkdownEditor.makeNSView — wire the click recognizer
------------------------------------------------------------------------------------------------
Add a click recognizer so a single click on a rendered checkbox toggles it. Two options:

   (A) Use the provided `TaskBoxClickTarget` (keeps the gesture target off the Coordinator):

        // in makeNSView, after `textView.delegate = context.coordinator` etc.
        let clickTarget = TaskBoxClickTarget(textView: textView, coordinator: context.coordinator)
        let click = NSClickGestureRecognizer(target: clickTarget,
                                             action: #selector(TaskBoxClickTarget.handleClick(_:)))
        click.numberOfClicksRequired = 1
        click.delaysPrimaryMouseButtonEvents = false   // don't swallow normal caret-placement clicks
        textView.addGestureRecognizer(click)
        // Retain the target for the view's lifetime. Simplest: store it on the EditorDocument, or use
        // objc_setAssociatedObject(textView, &key, clickTarget, .OBJC_ASSOCIATION_RETAIN). The gesture
        // recognizer's `target` is UNOWNED, so `clickTarget` MUST be retained elsewhere or clicks no-op.

   (B) Make the Coordinator itself the target (it already lives as long as the view). Add to Coordinator:

        @objc func handleTaskClick(_ gr: NSClickGestureRecognizer) {
            let p = gr.location(in: doc.textView)
            _ = toggleTaskBoxAt(doc.textView.characterIndexForInsertion(at: p))
        }

   and in makeNSView:

        let click = NSClickGestureRecognizer(target: context.coordinator,
                                             action: #selector(MarkdownEditor.Coordinator.handleTaskClick(_:)))
        click.delaysPrimaryMouseButtonEvents = false
        textView.addGestureRecognizer(click)

   Option (B) is the least bookkeeping (no extra retain) and is recommended; `TaskBoxClickTarget` is
   provided for teams who prefer to keep gesture handling out of the delegate.

   Click → index: `characterIndexForInsertion(at:)` returns an insertion index, which can resolve to the
   inner char or either side of it; `toggleTaskBoxAt` already probes `idx`, `idx-1`, `idx+1` and confirms
   via the scanner, so clicking anywhere on the rendered box toggles. Because the recognizer does NOT
   delay primary mouse events, a click that misses every box falls through to normal caret placement; a
   click that hits a box ALSO places the caret (harmless — the caret then sits next to the box and the
   raw `[ ]` reveals on that line, which is the expected editable state after toggling).

   Alternative to a gesture recognizer: override `mouseDown(_:)` on MarkdownTextView, compute
   `characterIndex(for: convert(event.locationInWindow, from: nil))` (or `characterIndexForInsertion`),
   call the coordinator's toggle, and `return` early on a hit (else `super.mouseDown`). The gesture
   recognizer is preferred because it doesn't interfere with selection/drag handling.

------------------------------------------------------------------------------------------------
 Summary of what the lead feeds where
------------------------------------------------------------------------------------------------
   • VM computes  : `vm.taskBoxes: [Int: Bool]`  in `recomputeHidden()` (+ inserts the two bracket indices
                     into `collector.hidden`), with the caret-line guard.
   • Glyph delegate reads `vm.taskBoxes`, builds `TaskBoxGlyphs(font:)` once, substitutes ☐/☑ for the
                     inner char; the brackets are already hidden via `hiddenChars`.
   • Click        : `makeNSView` adds an `NSClickGestureRecognizer` → `Coordinator.toggleTaskBoxAt(idx)`
                     (idx from `characterIndexForInsertion(at:)`).
   • Offset model : 3 source chars per box → 2 hidden (.null) + 1 substituted glyph = strict 1:1
                     char↔glyph; no offsets shift. Toggle is a single-char undoable replace.
================================================================================================
*/
