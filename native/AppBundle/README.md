...
`swift build -c release`, then assembles **`native/.build/Mallow.app`**
(wiped and rebuilt each run, so it's idempotent). If `build-app.sh` isn't
executable yet: `chmod +x native/build-app.sh`.

## Run

```sh
open native/.build/Mallow.app                       # launch
open -a native/.build/Mallow.app /path/to/file.md   # open a file (Finder-style)
```

`open native/.build/Mallow.app --args foo.md` does **not** pass the file —
...
- `xattr -dr com.apple.quarantine native/.build/Mallow.app`
...
Application: …" native/.build/Mallow.app`, then `xcrun notarytool submit … &&
...
```sh
otool -L native/.build/Mallow.app/Contents/MacOS/Mallow | grep -iv /System/Library
