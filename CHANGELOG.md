...
- **Typing `-` (or `=`) on the line under some text now shows the character.** That `-` is a Markdown *setext underline*; since 1.1.3 the line above stays a normal paragraph, but the `-` itself was still hidden, so it looked like your keystroke did nothing and the cursor jumped back up. The underline now shows as plain text (and becomes a bullet once you type a list item after it). The bare `#` of an empty heading you're still typing is shown for the same reason.
