...
- **Hidden markdown markers no longer take up invisible space.** The concealed syntax characters (backticks, `**`, `#`, link URLs, …) were invisible but still occupied their full width in the layout. That ghost space made every inline `code` pill about two characters wider than its text, left odd gaps around bold text and links, indented headings off the left margin, and knocked table columns out of alignment whenever a cell contained a marker. Hidden markers are now truly zero-width, so pills hug their text, headings align with the body, and marker-bearing table cells sit exactly on their columns.
- **Table cells now have even padding on both sides of every column rule — on every row.** Rules used to be positioned from an estimate that drifted off the actual laid-out text (text could kiss a rule on one side of a wide gap on the other), and individual rows could sit ~4pt off the rest of their column. Each rule is now centred in the real measured gap between two columns, and every row's cell is measured identically, so the whole table reads with consistent breathing room.
- **Inline `code` now sits at a natural width.** The ghost-width backticks above made every `code` span ~2 characters too wide; with that fixed, the pill hugs the text and gets a small, even 3.5pt of padding on each side, with the monospaced font at 0.9× the body size — neither bulging past the surrounding text nor cramped against the pill edges.
...
- **A marker you've typed but not yet given content now shows, instead of vanishing.** Typing a lone `-` / `*` / `+` / `1.` (a list bullet you're starting), a `-`/`=` on the line under some text (a *setext underline*), or a bare `#` (an empty heading) used to show nothing — the character was hidden as "syntax" before it actually formatted anything, so it looked like your keystroke did nothing (and the cursor could appear to jump up a line). These now show literally and turn into the styled element — a bullet, a heading — the moment you type content after them.
