...
- **Inline `code` is no longer too wide.** Two causes: the ghost-width backticks above, and the monospaced font sitting large — it now renders slightly smaller (0.85×), so a `code` span reads inline with the surrounding text instead of bulging past it.
