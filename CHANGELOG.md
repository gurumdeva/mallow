...
- **Inline HTML tags no longer merge the words around them.** Raw inline HTML such as `<br>`, `<span>…</span>`, `<sub>`, or `<kbd>` was being silently zero-width-hidden in the editor — so `line<br>wrap` displayed as `linewrap` and `H<sub>2</sub>O` as `H2O`. The text was always safe on disk (the file kept the exact bytes); it only vanished on screen because the editor hides syntax by the complement of what the parser marks as content, and the engine had no rule for inline HTML. The engine now treats inline HTML as content, so the literal tags show — consistent with how the editor already shows entities like `&amp;` verbatim (markdown-as-truth). A new property test pins the underlying guarantee that no on-screen content can be dropped this way.
