...
export class StatsCalculator {
  ...
  calculate(markdown: string, lang: Lang = 'en'): Stats {
    // 이미지 마크다운은 모든 통계(글자/단어/문단/읽기시간)에서 제외한다. 특히 붙여넣은
    // 이미지의 base64 data URI는 매우 긴 문자열이라, 포함하면 글자 수·읽기 시간이 비정상적으로
    // 부풀려지고(이미지 한 장에 "2594분"), 이미지만 있는 줄이 문단으로 잘못 세이기도 한다.
    // 네 단계로 제거한다(줄바꿈은 보존돼 문단 구조 유지):
    //  (1) 인라인 이미지 ![alt](url) — URL 안 괄호 1단계 중첩(예: Foo_(bar))까지 균형 괄호로 흡수.
    //  (2) 참조형 이미지 사용 ![alt][id] — 짧지만 산문이 아니므로 제외.
    //  (3) 링크/이미지 참조 정의 줄 `[id]: url ...` — 여기에 긴 data URI가 붙는 흔한 누락 경로였다.
    //  (4) 안전망: 위에서 못 걸러낸(예: 제목에 괄호 2단계 중첩) 잔여 data URI를 통째로 제거.
    //      (100자 이상 연속 토큰만 — 산문 속 짧은 "data:" 언급은 건드리지 않는다.)
    const withoutImages = markdown
      .replace(/!\[[^\]]*\]\(([^()]*(?:\([^()]*\)[^()]*)*)\)/g, '')
      .replace(/!\[[^\]]*\]\[[^\]]*\]/g, '')
      .replace(/^[ \t]{0,3}\[[^\]]+\]:[ \t]*\S.*$/gm, '')
      .replace(/data:[^\s)\]]{100,}/g, '')
